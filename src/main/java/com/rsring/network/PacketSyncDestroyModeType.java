 ...
 * 客户端 -> 服务端：同步销毁模式类型
 */
public class PacketSyncDestroyModeType implements IMessage {

    private int modeTypeId;

    public PacketSyncDestroyModeType() {}

    public PacketSyncDestroyModeType(DestroyModeType modeType) {
        this.modeTypeId = modeType.getId();
    }

    @Override
    public void fromBytes(ByteBuf buf) {
        modeTypeId = buf.readInt();
    }

    @Override
    public void toBytes(ByteBuf buf) {
        buf.writeInt(modeTypeId);
    }

    public static class Handler implements IMessageHandler<PacketSyncDestroyModeType, IMessage> {
        @Override
        public IMessage onMessage(PacketSyncDestroyModeType msg, MessageContext ctx) {
            ...
            player.getServerWorld().addScheduledTask(() -> {
                ...
                // 设置销毁模式类型
                DestroyModeType modeType = DestroyModeType.fromId(msg.modeTypeId);
                cap.setDestroyModeType(modeType);
