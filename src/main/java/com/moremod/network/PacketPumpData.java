    ...
    public static class Handler implements IMessageHandler<PacketPumpData, IMessage> {
        ...
        public IMessage onMessage(PacketPumpData msg, MessageContext ctx) {
            Minecraft.getMinecraft().addScheduledTask(() -> {
                if (Minecraft.getMinecraft().currentScreen instanceof com.moremod.client.GuiExperiencePumpController) {
                    ((com.moremod.client.GuiExperiencePumpController) Minecraft.getMinecraft().currentScreen).updateFromPacket(msg);
                }
            });
            return null;
        }
