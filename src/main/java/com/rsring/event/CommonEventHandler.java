...
public class CommonEventHandler {
    ...
    private ItemStack findExperiencePumpController(EntityPlayer player) {
        ...
        for (EnumHand hand : EnumHand.values()) {
            ItemStack heldStack = player.getHeldItem(hand);
            if (!heldStack.isEmpty() && heldStack.getItem() instanceof com.rsring.item.ItemExperiencePumpController) {
                return heldStack;
            }
        }
        ...
        for (int i = 9; i < 36; i++) {
            ItemStack stack = player.inventory.getStackInSlot(i);
            if (!stack.isEmpty() && stack.getItem() instanceof com.rsring.item.ItemExperiencePumpController) {
                return stack;
            }
        }
    ...
    private void syncControllerToTank(EntityPlayer player, ItemStack controllerStack, ItemStack tankStack) {
        ...
        // 从控制器获取配置
        com.rsring.item.ItemExperiencePumpController controllerItem = (com.rsring.item.ItemExperiencePumpController) controllerStack.getItem();
        ...
        // 应用到储罐
        com.rsring.capability.IExperiencePumpCapability cap = tankStack.getCapability(
        ...
        if (cap != null) {
            ...
            com.rsring.item.ItemExperiencePump.syncCapabilityToStack(tankStack, cap);
        }
    }
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (!controllerStack.isEmpty()) {
            ...
            com.rsring.experience.TankScanResult scan = pumpController.scanAllInventories(player);
            java.util.List<ItemStack> tanks = scan.getAllTanks();
            for (ItemStack tank : tanks) {
                com.rsring.capability.IExperiencePumpCapability cap = tank.getCapability(com.rsring.capability.ExperiencePumpCapability.EXPERIENCE_PUMP_CAPABILITY, null);
                if (cap == null) continue;
                cap.setMode(mode);
                cap.setRetainLevel(retainLevel);
                cap.setUseForMending(useForMending);
                com.rsring.item.ItemExperiencePump.syncCapabilityToStack(tank, cap);
            }
            // 控制器应主动执行泵送行为：如果模式为罐->人或人->罐，调用中央控制器执行一次按等级计算的泵送
            if (mode == com.rsring.capability.IExperiencePumpCapability.MODE_PUMP_TO_PLAYER ||
                mode == com.rsring.capability.IExperiencePumpCapability.MODE_PUMP_FROM_PLAYER) {
                // 计算需要移动的经验量（基于保留等级）
                ...
                } else if (mode == com.rsring.capability.IExperiencePumpCapability.MODE_PUMP_FROM_PLAYER) {
                    // 计算玩家超过保留等级可抽取的经验
    ...
    private void checkPlayerExperienceChange(EntityPlayer player, ItemStack pumpStack) {
        ...
        com.rsring.capability.IExperiencePumpCapability cap = pumpStack.getCapability(
            com.rsring.capability.ExperiencePumpCapability.EXPERIENCE_PUMP_CAPABILITY, null);
        if (cap == null) return;
        
        // 只在"从玩家泵入"模式下检测
        if (cap.getMode() != com.rsring.capability.IExperiencePumpCapability.MODE_PUMP_FROM_PLAYER) {
            lastPlayerXp.remove(player.getUniqueID());
            return;
        }
