 ...
 * 万级储罐彩蛋事件处理器
 *
 * 功能：将万级储罐与樱花维度物品合成，获得彩蛋储罐
 * 彩蛋储罐在饰品栏时增加5点幸运值
 */
public class AnvilTankEasterEggHandler {
    ...
    public void onAnvilUpdate(AnvilUpdateEvent event) {
        ...
        // 检查左侧是否为万级经验储罐
        if (!(left.getItem() instanceof ItemExperienceTank10000)) return;

        // 获取储罐能力
        IExperiencePumpCapability cap = left.getCapability(ExperiencePumpCapability.EXPERIENCE_PUMP_CAPABILITY, null);
        if (cap == null) return;
        ...
        IExperiencePumpCapability outputCap = output.getCapability(ExperiencePumpCapability.EXPERIENCE_PUMP_CAPABILITY, null);

        if (outputCap != null) {
            // 设置为彩蛋储罐
            outputCap.setEasterEgg(true);
            ExperiencePumpCapability.syncCapabilityToStack(output, outputCap);
        }
    ...
    public void onAnvilRepair(AnvilRepairEvent event) {
        ...
        if (!(output.getItem() instanceof ItemExperienceTank10000)) return;

        IExperiencePumpCapability cap = output.getCapability(ExperiencePumpCapability.EXPERIENCE_PUMP_CAPABILITY, null);
        if (cap == null) return;
        ...
        if (cap.isEasterEgg() && isNewEasterEgg) {
            ...
            if (player != null && !player.world.isRemote) {
                ...
                player.sendMessage(new TextComponentString(
                    TextFormatting.BLUE + "苍穹狂傲魔龙弑神战帝 " +
